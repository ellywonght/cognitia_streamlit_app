...
if st.session_state.step == 0:
    ...
    st.info("🧿 模擬眼球追蹤中... 系統將分析掃視穩定性與跳動頻率")
    st.markdown("""➡️ 目標向右移動中...
👁️ 眼球跟隨中...
✅ 偵測到掃視反應，追蹤穩定""")
    st.success("分析結果：掃視速度正常，建議定期觀察")
    st.session_state.session_log.append("眼球模擬：掃視正常")
    if st.button("下一步"):
        st.session_state.step += 1

elif st.session_state.step == 3:
    st.info("🔆 模擬PLR（瞳孔光反應）：假設光源閃爍，系統將模擬反應")
    st.markdown("""💡 光源：開啟
🕳️ 瞳孔大小：逐步縮小中...
✅ 縮小反應延遲約 0.23 秒，屬正常範圍""")
    st.success("結果：PLR 模擬反應良好")
